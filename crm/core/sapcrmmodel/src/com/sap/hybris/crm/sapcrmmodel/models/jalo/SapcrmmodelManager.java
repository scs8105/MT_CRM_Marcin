...
/**
 * This is the extension manager of the Sapcrmmodel extension.
 */
public class SapcrmmodelManager extends GeneratedSapcrmmodelManager {
    /**
     * Edit the local|project.properties to change logging behavior (properties
     * 'log4j.*').
     */
    private static final Logger LOG = Logger.getLogger(SapcrmmodelManager.class.getName());
     ...
     */
    public SapcrmmodelManager() // NOPMD
    {
        if (LOG.isDebugEnabled()) {
            LOG.debug("constructor of SapcrmmodelManager called.");
        }
    }
     ...
     */
    public static SapcrmmodelManager getInstance() {
        return (SapcrmmodelManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
                .getExtension(SapcrmmodelConstants.EXTENSIONNAME);
    }
    ...
    public void init() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("init() of SapcrmmodelManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("destroy() of SapcrmmodelManager called, current tenant: " + getTenant().getTenantID());
        }
    }
