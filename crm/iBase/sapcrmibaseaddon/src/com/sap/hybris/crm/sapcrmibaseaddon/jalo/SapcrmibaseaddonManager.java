 ...
 * /
 */
package com.sap.hybris.crm.sapcrmibaseaddon.jalo;

import java.util.Map;

import org.apache.log4j.Logger;

import com.sap.hybris.crm.sapcrmibaseaddon.constants.SapcrmibaseaddonConstants;

import de.hybris.platform.core.Registry;
import de.hybris.platform.util.JspContext;

/**
 * This is the extension manager of the Sapcrmibaseaddon extension.
 */
public class SapcrmibaseaddonManager extends GeneratedSapcrmibaseaddonManager {
    /**
     * Edit the local|project.properties to change logging behavior (properties
     * 'log4j.*').
     */
    private static final Logger LOG = Logger.getLogger(SapcrmibaseaddonManager.class.getName());

    /*
     * Some important tips for development:
     *
     * Do NEVER use the default constructor of manager's or items. => If you
     * want to do something whenever the manger is created use the init() or
     ...
     */
    public static SapcrmibaseaddonManager getInstance() {
        return (SapcrmibaseaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
                .getExtension(SapcrmibaseaddonConstants.EXTENSIONNAME);
    }
     ...
     */
    public SapcrmibaseaddonManager() // NOPMD
    {
        if (LOG.isDebugEnabled()) {
            LOG.debug("constructor of SapcrmibaseaddonManager called.");
        }
    }
    ...
    public void init() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("init() of SapcrmibaseaddonManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("destroy() of SapcrmibaseaddonManager called, current tenant: " + getTenant().getTenantID());
        }
    }
