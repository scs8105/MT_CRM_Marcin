...
<div class="account-section-header">
	<spring:theme code="text.account.complaints.updatecomplaint.header"
		text="Update Complaint" arguments="${ticketData.id}"/>
</div>
	...
	<div class="col-md-8 col-xs-10">
		<div class="update-section">
			<form:form method="post" commandName="sapCrmComplaintForm">
				...
				<c:if test="${not empty ticketData.associatedOrderCode}">
					<div class="form-group">
						<label class="control-label"><spring:theme
								code="text.account.complaints.associatedOrder"
								text="Associated Order" /></label>
						<p class="form-control-static">${ticketData.associatedOrderCode}</p>
					</div>
				</c:if>
				<c:if test="${not empty ticketData.associatedOrderEntryCodes}">
					<div class="form-group">
						<label class="control-label"><spring:theme
								code="text.account.complaints.associatedOrderEntries"
								text="Associated Order Entries" /></label>
						<c:forEach items="${ticketData.associatedOrderEntryCodes}"
							var="entry">
							<p class="form-control-static">${entry}</p>
						</c:forEach>
					</div>
				</c:if>
			...
			<div class="buttons">
				<a href="../crm-support-tickets" class="btn btn-default"> <spring:theme
					...
					<ycommerce:testId code="supportTicket_update_button">
						...
						<button class="btn btn-primary" id="updateComplaint" type="submit">
							<spring:theme
